...
private enum LintToolError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .usage:
            return "usage: orbi-swiftlint <request.json>"
        case let .invalidWorkingDirectory(path):
            return "failed to change directory to \(path)"
        case .invalidConfigurationPayload:
            return "Orbi lint configuration must be a JSON object"
        }
    }
}

public func orbiSwiftLintMain(arguments: [String] = CommandLine.arguments) -> Int32 {
    do {
        return try OrbiSwiftLintTool.run(arguments: arguments)
    } catch {
...
@_cdecl("orbi_swiftlint_run_request")
public func orbi_swiftlint_run_request(requestPath: UnsafePointer<CChar>?) -> Int32 {
    guard let requestPath else {
        writeToStandardError("error: missing request path for orbi-swiftlint\n")
        return 1
    }
    return orbiSwiftLintMain(arguments: ["orbi-swiftlint", String(cString: requestPath)])
}

private enum OrbiSwiftLintTool {
    ...
    private static func registerRules() {
        ...
        // so Orbi registers the public rule sets it can see up front.
    ...
    private static func loadConfiguration(from json: String?) throws -> ResolvedLintConfiguration {
        ...
        guard let orbiConfiguration = payload as? [String: Any] else {
            throw LintToolError.invalidConfigurationPayload
        }

        let translated = translateOrbiConfiguration(orbiConfiguration)
        return ResolvedLintConfiguration(
    ...
    private static func translateOrbiConfiguration(
        _ orbiConfiguration: [String: Any]
        ...
        for (key, value) in orbiConfiguration {
...
private func maybeHandleMockRequest(at requestPath: String) throws -> Int32? {
    ...
    appendToMockLog(
        mockLogPath,
        lines: [
            "orbi-swiftlint \(requestPath)\n",
            "orbi-swiftlint request:\n",
            requestBody,
            "\n",
        ]
    )
